...
::tip
NuxtHub uses libsql and local file storage during development by default. Use Cloudflare dev emulation only when you explicitly set `nitro.preset: 'cloudflare_module'` in your `nuxt.config.ts`.
::
...
::tabs{sync="provider"}
  ...
  **R2 Buckets:**
  ...
  ::tip
  Add storage through the [Vercel Marketplace](https://vercel.com/marketplace) (Neon for Postgres, Upstash for Redis, Vercel Blob). By default, these connect to all environments—create separate storage instances per environment for isolation.
  ::
