...
2. Set the same `NUXT_HUB_PROJECT_SECRET_KEY` and `NUXT_HUB_PROJECT_URL` environment variables in your local project
    ...
    ```
3. (Optional) If you are switching from NuxtHub Admin to self-hosting, remove the `NUXT_HUB_PROJECT_KEY` and `NUXT_HUB_PROJECT_DEPLOY_TOKEN` environment variables from the Cloudflare project.
