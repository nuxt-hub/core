...
When self hosting, to use AI features such as `hubAI()` and `hubAutoRAG()` during local development, you need to configure additional environment variables within your `.env` file:
...
::field-group
  ...
  ::
  ::field{name="NUXT_HUB_CLOUDFLARE_API_TOKEN"}
    Your Cloudflare API token with **Workers AI:Read** permissions
  ::
::
