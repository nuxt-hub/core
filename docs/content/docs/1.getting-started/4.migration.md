...
For Cloudflare D1, NuxtHub generates the Wrangler migration metadata for you, but `wrangler deploy` and Cloudflare Git / Workers Builds do not apply D1 migrations automatically. Add a separate migration step before deploy.
::

3. Deploy using [Workers Builds](https://developers.cloudflare.com/workers/ci-cd/builds/) or [Pages CI](https://developers.cloudflare.com/pages/configuration/git-integration/github-integration/). If you use D1, follow the [CI/CD D1 migration workflow](/docs/guides/ci-cd#d1-migrations-in-cicd).
...
- Use the [Claude Code migration skill](https://github.com/onmax/nuxthub-v1-skill) for AI-assisted migration
