...
NuxtHub support filtering files by dialect by using the `.{dialect}.ts` suffix, e.g. `server/db/schema.postgresql.ts` will be only imported if `hub.db` is `postgresql`.
::
...
::important
Make sure to run `npx nuxt hub db generate` to generate the database migrations each time you change your database schema and restart the development server.
::
...
- **Generate Migrations**: Use `npx nuxt hub db generate` to automatically generate database migrations from schema changes
...
- **Workflow**:
  ...
  2. Run `npx nuxt hub db generate` to generate the migration
  3. Run `npx nuxt hub db migrate` to apply the migration to the database, or run `npx nuxt dev` to apply the migration during development
