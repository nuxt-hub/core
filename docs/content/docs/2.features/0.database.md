...
The `hub:database` module provides access to the database through a [Drizzle ORM](https://orm.drizzle.team) instance.
...
::tip
`db` is auto-imported on server-side, you can directly use it without importing it from `hub:database`.
::
