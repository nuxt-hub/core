...
### Self-hosted Cache Management

When self-hosting your application and using the NuxtHub Admin UI for cache management (viewing/deleting cache entries), you need to configure additional environment variables to allow direct Cloudflare API access:
...
NUXT_HUB_CLOUDFLARE_CACHE_NAMESPACE_ID=your-kv-namespace-id
```

::tip
You can find your KV namespace ID in the [Cloudflare Dashboard](https://dash.cloudflare.com) under **Workers & Pages** → **KV**.
::

::note
The API token needs to have **Workers KV Storage Read and Write** permissions for the namespace. These environment variables are only required if you use the NuxtHub Admin UI for cache management through API routes (like `/api/_hub/cache/*`). Normal cache operations (reading/writing cache) work directly through the KV binding.
::
