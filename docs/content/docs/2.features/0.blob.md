...
NuxtHub automatically configures the blob storage driver based on your hosting provider.
...
::tabs{sync="provider"}
  ...
  :::tabs-item{label="Netlify" icon="i-simple-icons-netlify"}
    ...
    When deploying to Netlify, it automatically configures [Netlify Blobs](https://docs.netlify.com/build/data-and-storage/netlify-blobs/).
  ...
  :::tabs-item{label="Other" icon="i-simple-icons-nodedotjs"}
    ...
    ::tip{to="#custom-driver"}
      You can configure `blob` to use a different storage driver.
    ::
...
You can use any [unstorage](https://unstorage.unjs.io/drivers) driver by providing a configuration object to `blob`.
...
Server import that returns a set of methods to manipulate the blob storage.
