...
| Variable | Description | Priority |
| --- | --- | --- |
| `UPSTASH_REDIS_REST_URL` | Upstash Redis REST URL | 1st |
| `UPSTASH_REDIS_REST_TOKEN` | Upstash Redis REST token | - |
| `KV_REST_API_URL` | Vercel KV REST URL (auto-detected on Vercel) | 2nd |
...
| Variable | Description | Priority |
| --- | --- | --- |
| `REDIS_URL` | Redis connection URL | 1st |
| `KV_URL` | Alternative Redis URL (Vercel) | 2nd |
...
### KV
`UPSTASH_REDIS_REST_URL` → `KV_REST_API_URL` → `REDIS_URL` → `KV_URL` → filesystem
