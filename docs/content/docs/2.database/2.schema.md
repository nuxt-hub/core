...
The relations are exported and can be imported similarly to the schema:

```ts
// Import relations
import { relations } from '@nuxthub/db/relations'

// Also possible: Import relations form main module
import { relations } from '@nuxthub/db'
```
...
// Relations export — named "postsRelations" to avoid conflicts
...
Types inferred from your database schema and relations are only available on the server-side by default. To share these types with your Vue application, you can use the [`shared/`](https://nuxt.com/docs/guide/directory-structure/shared) directory which is auto-imported across both server and client.
...
```
::important
Make sure to import the schema (or parts of it) and relations form `'@nuxthub/db/schema'` and `'@nuxthub/db/relations'` directly.
When importing from `'@nuxthub/db'` the db instantiation may also be run on the client.
::
