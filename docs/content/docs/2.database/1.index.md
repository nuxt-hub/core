...
NuxtHub supports read replicas using Drizzle ORM's [`withReplicas()`](https://orm.drizzle.team/docs/read-replicas) feature. When a replica URL environment variable is detected, read queries are automatically routed to the replica while write queries go to the primary database.

Set the appropriate environment variable for your database:

::tabs{sync="database-dialect"}
  :::tabs-item{label="PostgreSQL" icon="i-simple-icons-postgresql"}
  | Variable | Description |
  | --- | --- |
  | `DATABASE_URL_REPLICA` | Connection URL for the read replica. Also checks `POSTGRES_URL_REPLICA` and `POSTGRESQL_URL_REPLICA`. |
  :::
  :::tabs-item{label="MySQL" icon="i-simple-icons-mysql"}
  | Variable | Description |
  | --- | --- |
  | `DATABASE_URL_REPLICA` | Connection URL for the read replica. Also checks `MYSQL_URL_REPLICA`. |
  :::
  :::tabs-item{label="SQLite" icon="i-simple-icons-sqlite"}
  | Variable | Description |
  | --- | --- |
  | `TURSO_DATABASE_URL_REPLICA` | Connection URL for the read replica. Also checks `LIBSQL_URL_REPLICA` and `DATABASE_URL_REPLICA`. |
  | `TURSO_AUTH_TOKEN_REPLICA` | Auth token for the replica (optional, falls back to primary token). Also checks `LIBSQL_AUTH_TOKEN_REPLICA`. |
  :::
::

::callout{icon="i-lucide-info"}
For `postgres-js` and `mysql2` drivers, read replicas are only enabled in production builds. The `neon-http` and `libsql` drivers support replicas in both development and production.
::
