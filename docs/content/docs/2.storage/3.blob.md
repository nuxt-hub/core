...
::note
This is a composable to be used in the Vue side of your application (not the `server/` directory).
::

Application composable that creates a multipart upload helper.

```ts [utils/multipart-upload.ts]
export const upload = useMultipartUpload('/api/files/multipart')
```
