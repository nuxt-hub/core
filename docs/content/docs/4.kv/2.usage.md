...
Redis and Upstash expose two additional operations for single-use values and counters:
...
These methods are unavailable with Deno KV, Cloudflare KV, S3, and local filesystem storage. Check that the method exists or configure Redis or Upstash before using it.
