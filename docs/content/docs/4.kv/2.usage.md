...
Redis, Upstash, and Deno KV expose two additional operations for single-use values and counters:

```ts
import { kv } from '@nuxthub/kv'

if (!kv.getAndDelete || !kv.increment) {
  throw new Error('This KV driver does not support atomic operations')
}

const token = await kv.getAndDelete('verification:token')
const attempts = await kv.increment('rate-limit:user', 60)
```

`getAndDelete()` returns and removes a value in one operation. `increment()` returns the post-increment value and applies its TTL only when the counter is created.

These methods are unavailable with Cloudflare KV, S3, and local filesystem storage because those drivers cannot provide the required atomic guarantee. Check that the method exists or configure an atomic driver before using it.
