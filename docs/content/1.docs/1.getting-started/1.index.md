...
- **SQL database** to store your application's data with [`hubDatabase()`](/docs/features/database)
- **Key-Value** to store JSON data accessible globally with low-latency with [`hubKV()`](/docs/features/kv)
- **Blob storage** to store static assets, such as images, videos and more with [`hubBlob()`](/docs/features/blob)
