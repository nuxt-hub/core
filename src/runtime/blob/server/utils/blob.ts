...
interface HubBlob {
   ...
   *
   * @example ```ts
   * const object = await blob.put('/my-image.jpg', file)
   * ```
   */
   ...
   *
   * @example ```ts
   * const metadata = await blob.head('/my-image.jpg')
   * ```
   */
   ...
   *
   * @example ```ts
   * const body = await blob.get('/my-image.jpg')
   * ```
   */
...
export const blob = {
  ...
  delete: async (pathnames: string | string[]) => {
    await blob.del(pathnames)
  },
