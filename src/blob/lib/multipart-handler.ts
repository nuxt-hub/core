...
import type { BlobMultipartOptions, BlobStorage } from '../types'
...
export function createMultipartUploadHandler(blob: BlobStorage) {
  if (blob.driver.name === 'vercel-blob') {
    return async (event: H3Event, options?: BlobMultipartOptions) =>
      import('./multipart/vercel')
        .then(({ multipartUploadHandler }) => multipartUploadHandler(event, options))
  }

  return createMultipartUploadHandlerWithBlobStorage(blob)
}

function createMultipartUploadHandlerWithBlobStorage(blob: BlobStorage) {
