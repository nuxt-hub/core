...
async function generateDatabaseSchema(nuxt: Nuxt, hub: ResolvedHubConfig) {
  ...
  nuxt.options.alias ||= {};
  // Keep schema imports resolvable without requiring a physical package during prepare/dev/typecheck.
  addTypeTemplate(
    {
...
declare module 'hub:db' {
  const db: ReturnType<typeof drizzleCore<DbSchema>>
...
${
  ...
  const dbAliasTemplate = addTemplate({
    filename: "hub/db.mjs",
    write: true,
    getContents: () => `export { db, schema } from './db.${dbHash}.mjs'\n`,
  });

  nuxt.options.alias ||= {};
  nuxt.options.alias["@nuxthub/db"] = dbShimTemplate.dst;
  nuxt.options.alias["hub:db"] = dbAliasTemplate.dst;
...
`;
  ...
  nuxt.hook("nitro:build:before", async () => {
    ...
    const physicalDbContent = drizzleOrmContent.replace(
    ...
    const schemaTypesSource = join(
      nuxt.options.buildDir,
      "hub/db/schema.d.mts",
    );

    const schemaCopied = await copyFile(schemaSource, schemaPath)
      .then(() => true)
      .catch(() => false);
    const schemaTypesCopied = await copyFile(schemaTypesSource, schemaDtsPath)
      .then(() => true)
      .catch(() => false);
